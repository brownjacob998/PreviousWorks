...
void CProject1Dlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT_REPORT, _report);
}
...
{
	_comboDateRange.AddString(_T("All reports"));
...
void CProject1Dlg::_addReportToList(IMailPtr &mail, CString &filePath)
{
	CString sender;
...
void CProject1Dlg::OnBnClickedButtonStart()
{
	...
	try
	{
		...
		for (int i = 0; i < mailInfos->Count; i++)
		{
			...
			status.Format(_T("Retrieving headers %d/%d ..."), i + 1, mailInfos->Count);
			_statusBar.GetStatusBarCtrl().SetText(status.GetString(), 255, 0);
			...
			status.Format(_T("Report found, retrieve full data %d/%d ..."), i + 1, mailInfos->Count);
			mail = _client->GetMail(info);
			mail->SaveAs(fullPath.GetString(), VARIANT_TRUE);

			_addReportToList(mail, fullPath);
...
//========================================================================
// End Region: Retrieve reports
//========================================================================

//========================================================================
// Region: Parse and display reports
//========================================================================
...
void CProject1Dlg::_showReport(LPCTSTR lpszFile)
{
	try
	{
		IMailPtr mail;
		...
		if (mail->IsReport == VARIANT_FALSE)
		{
			_report.SetWindowText(_T("This is not a report"));
			return;
		}

		CString buffer;

		IMailReportPtr mailReport = mail->GetReport();
		switch (mailReport->ReportType)
		{
		case DeliveryReceipt:
			buffer = _T("This is a deliver receipt.\r\n\r\n");
			break;
		case ReadReceipt:
			buffer = _T("This is a read receipt.\r\n\r\n");
			break;
		case Receipt_Deleted:
			buffer = _T("This is a unread receipt, this email was deleted without read!\r\n\r\n");
			break;
		case DelayedReport:
			buffer = _T("This is a delayed notification, this email will be tried later by server automatically!\r\n\r\n");
			break;
		default:
			buffer = _T("This is a failure report.\r\n\r\n");
			break;
		}

		buffer.AppendFormat(_T("Original Sender: %s\r\n"), (const TCHAR*)mailReport->OriginalSender);
		buffer.AppendFormat(_T("Original Recipient: %s\r\n"), (const TCHAR*)mailReport->OriginalRecipient);
		buffer.AppendFormat(_T("Original Message-ID: %s\r\n\r\n"), (const TCHAR*)mailReport->OriginalMessageID);

		if (mailReport->ReportType == FailureReport || mailReport->ReportType == DelayedReport)
		{
			buffer.AppendFormat(_T("Original Subject: %s\r\n"), (const TCHAR*)mailReport->OriginalSubject);
			buffer.AppendFormat(_T("Report MTA: %s\r\n"), (const TCHAR*)mailReport->ReportMTA);
			buffer.AppendFormat(_T("Error Code: %s\r\n"), (const TCHAR*)mailReport->ErrCode);
			buffer.AppendFormat(_T("Error Description: %s\r\n\r\n"), (const TCHAR*)mailReport->ErrDescription);
			buffer.Append(_T("---- Original Message Header ----\r\n\r\n"));

			IHeaderCollectionPtr headers = mailReport->OriginalHeaders;
			for (int i = 0; i < headers->Count; i++)
			{
				IHeaderItemPtr header = headers->Item(i);
				buffer.AppendFormat(_T("%s: %s\r\n"), (const TCHAR*)header->HeaderKey, (const TCHAR*)header->HeaderValue);
			}
		}

		_report.SetWindowText(buffer);
...
//========================================================================
// End Region: Parse and display reports
//========================================================================
...
void CProject1Dlg::OnSize(UINT nType, int cx, int cy)
{
	...
	_offsetChildWindow(GetDlgItem(IDC_EDIT_REPORT), 0, 0, dx, dy);
