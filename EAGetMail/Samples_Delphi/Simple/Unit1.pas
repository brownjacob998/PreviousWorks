...
type
  ...
  private
    ...
    Function AddNewMailToListMail(oMail: IMail; filePath: WideString): TListItem;
...
begin
  ...
  lstDateRange.AddItem('All emails', nil);
...
Function TForm1.AddNewMailToListMail(oMail: IMail; filePath: WideString): TListItem;
var
  item: TListItem;
begin
...
begin
  ...
  try
    ...
    for i := 0 to mailInfos.Count - 1 do
      begin
        textStatus.SimpleText := Format('Receiving %d/%d ...', [i+1, mailInfos.Count]);
        ...
        if oUIDLManager.FindUIDL(oServer.DefaultInterface, oInfo.UIDL) = nil then
          begin
            ...
            if chkLeaveCopy.Checked Then
              ...
              begin
                textStatus.SimpleText := Format('Deleting %d ... ', [i+1]);
...
// ----------------------------------------------------
procedure TForm1.LoadLocalMails();
var
  oMail: TMail;
  i, UBound: integer;
...
begin
  ...
  begin
    ...
    AddNewMailToListMail(oMail.DefaultInterface, fullFileName);
...
begin
  result := LeftStr(filePath, length(filePath) - 4) + '.htm';
end;
