...
namespace EASendMail.Oauth
{
    public class OauthDesktopWrapper
    {
        ...
        OauthProviderInterface _provider = null;
        ...
        string _postString(string uri, string requestData)
        {
            ...
            IAsyncResult requestAsyncResult = httpRequest.BeginGetRequestStream(null, null);
            while ((!requestAsyncResult.AsyncWaitHandle.WaitOne(5, false)))
            {
                Application.DoEvents();
            }

            using (Stream requestStream = httpRequest.EndGetRequestStream(requestAsyncResult))
            {
            ...
            IAsyncResult responseAsyncResult = httpRequest.BeginGetResponse(null, null);
            while ((!responseAsyncResult.AsyncWaitHandle.WaitOne(5, false)))
            {
                Application.DoEvents();
            }
            
            HttpWebResponse httpResponse = httpRequest.EndGetResponse(responseAsyncResult) as HttpWebResponse;
