...
#Region "Create SmtpMail And SmtpServer instance based On Settings Of Form Controls"
    ...
    Private Function _createMail() As SmtpMail
        ...
        If String.Compare(mail.From.Address, _oauthWrapper.OauthProvider.UserEmail, True) <> 0 Then
...
#End Region
    ...
    Private Sub ButtonSend_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ButtonSend.Click
        If TextBoxFrom.Text.Length = 0 Then
            MessageBox.Show("You don't input from address, your Gmail email address will be used automatically.")
        End If
        ...
        ButtonClearToken.Enabled = (_oauthWrapper.OauthProvider.AccessToken.Length <> 0)
    End Sub
...
#Region "Html Editor"
    ...
    Private Sub HtmlEditor_Navigated(ByVal sender As Object, ByVal e As WebBrowserNavigatedEventArgs) Handles HtmlEditor.Navigated
        ...
        s.Append("<div>This sample demonstrates how to send html email using Gmail Oauth.</div><div>&nbsp;</div>")
        s.Append("<div>From: [$from]</div>")
        s.Append("<div>To: [$to]</div>")
        s.Append("<div>Subject: [$subject]</div><div>&nbsp;</div>")
        s.Append("<div>This sample project demonstrates how to send email using Gmail XOAUTH2, ")
        s.Append("Please create your client_id and client_secret introduced in Form GoogleOauthProvider.cs.</div>")
