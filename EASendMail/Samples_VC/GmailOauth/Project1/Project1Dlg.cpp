...
BOOL CProject1Dlg::OnInitDialog()
{
	...
	m_subject = _T("Test sample for Gmail OAUTH");
	_initCharset();
...
void CProject1Dlg::OnBnClickedOk()
{
	...
	m_oSmtp->AuthType = 5; // Gmail OAHT2;
...
void CProject1Dlg::NavigateComplete2Explorer1(LPDISPATCH pDisp, VARIANT* URL)
{
	...
	CString  s = _T("");
	s.Append(_T("<div>This sample demonstrates how to send html email using Gmail OAUTH.</div><div>&nbsp;</div>"));
	s.Append(_T("<div>From: [$from]</div>"));
	s.Append(_T("<div>To: [$to]</div>"));
	s.Append(_T("<div>Subject: [$subject]</div><div>&nbsp;</div>"));
	s.Append(_T("<div>This sample project demonstrates how to send email using Gmail OAUTH, "));
	s.Append(_T("Please create your client_id and client_secret introduced in Form GmailOAUTH.</div>"));
